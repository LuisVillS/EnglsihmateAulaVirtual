  ...
  try {
    ...
    if (session.valid && session.slug === params?.slug && session.manifestId === body?.manifestId) {
      ...
      const state = await saveFlipbookProgress({
        ...
        savedPageId: body?.savedPageId,
        savedPageIndex: body?.savedPageIndex,
    ...
    console.error("POST /api/library/books/[slug]/flipbook-bookmark failed", error);
    return NextResponse.json(
      { error: error?.message || "No se pudo guardar el bookmark del flipbook." },
      { status: 500 }
    );
  }
}
