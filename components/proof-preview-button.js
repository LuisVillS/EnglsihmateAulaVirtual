...
export default function ProofPreviewButton({
  preEnrollmentId,
  ...
  const proofUrl = useMemo(
    () => `/api/admin/prematriculas/${encodeURIComponent(preEnrollmentId)}/proof`,
    [preEnrollmentId]
  );
    ...
    <>
      ...
      {open ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="relative h-[88vh] w-full max-w-5xl rounded-2xl border border-border bg-surface shadow-2xl">
            <button
              type="button"
