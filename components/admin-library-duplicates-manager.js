    ...
    >
      <div className="flex flex-wrap items-center gap-2">
        <span className="rounded-lg border border-border bg-surface-2 px-2.5 py-1 text-[11px] font-semibold text-muted">
          {record.recordScope === "book" ? "Published" : "Staging"}
        </span>
      ...
      <p className="mt-1 text-sm text-muted">{record.authorDisplay || record.author_display || "Unknown author"}</p>
      <div className="mt-3 grid gap-1 text-xs text-muted">
        <p>Work: {record.openlibraryWorkKey || record.openlibrary_work_key || "N/A"}</p>
        <p>Archive: {record.internetArchiveIdentifier || record.internet_archive_identifier || "N/A"}</p>
        <p>Access: {record.ebookAccess || record.ebook_access || "N/A"}</p>
      </div>
      <span className="mt-4 inline-flex rounded-lg border border-border px-3 py-1 text-xs font-semibold text-foreground">
        {selected ? "Canonical selected" : "Select as canonical"}
      </span>
    </button>
  );
}
  ...
  return (
    <article className="space-y-4 rounded-2xl border border-border bg-surface p-5 shadow-sm">
      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          <p className="text-xs uppercase tracking-[0.24em] text-muted">Duplicate group</p>
        ...
        >
          {pending ? "Resolving..." : "Apply canonical choice"}
        </button>
      </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-border bg-surface p-5 shadow-sm">
        <div>
          <p className="text-xs uppercase tracking-[0.24em] text-muted">Resolution queue</p>
          <h1 className="mt-2 text-2xl font-semibold text-foreground">{groups.length} duplicate group(s)</h1>
        </div>
        <button
          type="button"
          onClick={runScan}
        ...
        >
          {pending ? "Scanning..." : "Re-scan duplicates"}
        </button>
      </div>
      ...
      {groups.length ? (
        ...
        <div className="rounded-2xl border border-dashed border-border bg-surface px-6 py-12 text-center">
          <p className="text-lg font-semibold text-foreground">No duplicate groups detected</p>
          <p className="mt-2 text-sm text-muted">Run a scan after importing or publishing new books.</p>
        </div>
      )}
    </div>
  );
