...
export async function loadTeacherStudentsOverview({ db, filters = {} }) {
  ...
  const [progressRows, historyRows, overrideRows, gradeRows, commissionsResult] = await Promise.all([
  ...
  const rows = sortedStudents.map((student) => {
    ...
    const currentGrade = gradeForLevel != null ? gradeForLevel : clampScore(student.student_grade);
...
export async function closeStudentLevel({
  ...
  const grade = clampScore(gradeRow?.admin_grade_0_100) ?? profileData.fallbackGrade ?? 0;
