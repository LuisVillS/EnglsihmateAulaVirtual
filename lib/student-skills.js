...
export const SKILL_TAG_VALUES = ["speaking", "reading", "grammar", "listening", "vocabulary"];
...
const STUDENT_SKILLS_MISSING_COLUMNS_CACHE_KEY = "__englishmateStudentSkillsMissingColumns";

function getStudentSkillsMissingColumnsCache() {
  if (!globalThis[STUDENT_SKILLS_MISSING_COLUMNS_CACHE_KEY]) {
    globalThis[STUDENT_SKILLS_MISSING_COLUMNS_CACHE_KEY] = new Map();
  }
  return globalThis[STUDENT_SKILLS_MISSING_COLUMNS_CACHE_KEY];
}

function filterKnownMissingColumns(tableName, columns = []) {
  const cache = getStudentSkillsMissingColumnsCache();
  const missingColumns = cache.get(tableName) || new Set();
  return columns.filter((column) => !missingColumns.has(column));
}

function rememberMissingColumn(tableName, columnName) {
  if (!tableName || !columnName) return;
  const cache = getStudentSkillsMissingColumnsCache();
  const missingColumns = cache.get(tableName) || new Set();
  missingColumns.add(columnName);
  cache.set(tableName, missingColumns);
}
...
function deriveSkillTagFromExerciseType(type) {
  ...
  if (normalizedType === "image_match" || normalizedType === "pairs" || normalizedType === "reading_exercise") {
    return "reading";
  }
...
function buildSkillAverageMap(progressRows = [], currentLevel = "") {
  ...
  return {
    ...
    listening: average(bucket.listening),
    vocabulary: average(bucket.vocabulary),
  };
}
...
function buildHistoryAverageMap(historyRows = [], currentLevel = "") {
  ...
  return {
    ...
    vocabulary: average(previousRows.map((row) => row.final_vocabulary_0_100)),
    grade: average(previousRows.map((row) => row.final_grade_0_100)),
    count: previousRows.length,
  };
}

function resolveManualSpeakingValue(row) {
  const speaking = clampScore(row?.speaking_value_0_100);
  if (speaking != null) return speaking;
  return clampScore(row?.listening_value_0_100);
}
...
function buildStudentSkillsBundle({
  ...
  const speakingOverride = pickRowByLevel(overrideRows, currentLevel);
  const manualSpeaking = resolveManualSpeakingValue(speakingOverride);
  const currentSpeaking = manualSpeaking ?? clampScore(current.speaking);
  const currentListening = clampScore(current.listening);
  const currentVocabulary = clampScore(current.vocabulary);

  const combined = {
    speaking: mergeSkillScore(currentSpeaking, history.speaking),
    ...
    vocabulary: mergeSkillScore(currentVocabulary, history.vocabulary),
  };
...
async function loadStudentLevelHistoryRowsByUserIds(db, userIds = [], { includeId = false } = {}) {
  const ids = Array.from(new Set((userIds || []).map((value) => cleanText(value)).filter(Boolean)));
  if (!ids.length) return [];

  let columns = filterKnownMissingColumns("student_level_history", [
    "user_id",
    "level",
    "started_at",
    "completed_at",
    "final_grade_0_100",
    "final_speaking_0_100",
    "final_reading_0_100",
    "final_grammar_0_100",
    "final_listening_0_100",
    "final_vocabulary_0_100",
    "notes",
    "updated_at",
  ]);
  if (includeId) columns.unshift("id");
  ...
  for (let attempt = 0; attempt < 6; attempt += 1) {
    ...
    if (!error) return data || [];
    if (isMissingObjectError(error, "student_level_history")) return [];

    const missingColumn = getMissingColumnFromError(error);
    if (missingColumn && columns.includes(missingColumn)) {
      rememberMissingColumn("student_level_history", missingColumn);
      columns = columns.filter((column) => column !== missingColumn);
      continue;
    }
    throw new Error(error.message || "No se pudo cargar historial de niveles.");
  }

  return [];
}

async function loadStudentSkillOverridesByUserIds(db, userIds = [], { includeId = false } = {}) {
  const ids = Array.from(new Set((userIds || []).map((value) => cleanText(value)).filter(Boolean)));
  if (!ids.length) return [];

  let columns = filterKnownMissingColumns("student_skill_overrides", [
    "user_id",
    "level",
    "speaking_value_0_100",
    "listening_value_0_100",
    "updated_by",
    "updated_at",
  ]);
  if (includeId) columns.unshift("id");
  ...
  for (let attempt = 0; attempt < 6; attempt += 1) {
    ...
    if (!error) return data || [];
    if (isMissingObjectError(error, "student_skill_overrides")) return [];

    const missingColumn = getMissingColumnFromError(error);
    if (missingColumn && columns.includes(missingColumn)) {
      rememberMissingColumn("student_skill_overrides", missingColumn);
      columns = columns.filter((column) => column !== missingColumn);
      continue;
    }
    throw new Error(error.message || "No se pudo cargar overrides de speaking.");
  }
...
export async function loadTeacherStudentsOverview({ db, filters = {} }) {
  ...
  const [progressRows, historyRows, overrideRows, gradeRows, quizAttemptRows, commissionsResult] = await Promise.all([
    ...
    loadStudentLevelHistoryRowsByUserIds(db, userIds),
    loadStudentSkillOverridesByUserIds(db, userIds),
  ...
  const rows = sortedStudents.map((student) => {
    ...
    return {
      ...
      speaking_current: bundle.current?.speaking ?? null,
      speaking_manual: bundle.manual?.speaking ?? null,
...
export async function loadTeacherStudentProfile({ db, studentId }) {
  ...
  const [progressRows, historyRows, overrideRows, gradeRows, quizAttempts] = await Promise.all([
    ...
    loadStudentLevelHistoryRowsByUserIds(db, [student.id], { includeId: true }),
    loadStudentSkillOverridesByUserIds(db, [student.id], { includeId: true }),
    loadRowsByUserIdsWithFallback({
      db,
      table: "student_course_grades",
...
export async function setStudentSpeakingOverride({
  db,
  actorId,
  userId,
  level,
  ...
  const speaking = clampScore(speakingValue);
  if (speaking == null) throw new Error("Speaking debe estar entre 0 y 100.");
  ...
  let data = null;
  let error = null;
  const missingSkillOverrideColumns = getStudentSkillsMissingColumnsCache().get("student_skill_overrides") || new Set();
  let useLegacyListeningColumn = missingSkillOverrideColumns.has("speaking_value_0_100");
  let mirrorLegacyListeningColumn = !missingSkillOverrideColumns.has("listening_value_0_100");

  for (let attempt = 0; attempt < 4; attempt += 1) {
    const payload = useLegacyListeningColumn
      ? {
        ...basePayload,
        listening_value_0_100: speaking,
      }
      : {
        ...basePayload,
        speaking_value_0_100: speaking,
        ...(mirrorLegacyListeningColumn ? { listening_value_0_100: speaking } : {}),
      };

    const result = await db
      .from("student_skill_overrides")
      .upsert(payload, { onConflict: "user_id,level" })
      .select("user_id, level, speaking_value_0_100, listening_value_0_100, updated_by, updated_at")
      .maybeSingle();
    data = result.data;
    error = result.error;
    ...
    if (!error) break;
    const missingColumn = getMissingColumnFromError(error);
    if (!useLegacyListeningColumn && missingColumn === "speaking_value_0_100") {
      rememberMissingColumn("student_skill_overrides", missingColumn);
      useLegacyListeningColumn = true;
      continue;
    }
    if (!useLegacyListeningColumn && mirrorLegacyListeningColumn && missingColumn === "listening_value_0_100") {
      mirrorLegacyListeningColumn = false;
      continue;
    }
    break;
  }
  ...
  if (error) {
    ...
    throw new Error(error.message || "No se pudo actualizar speaking.");
  }

  return {
    ...(data || basePayload),
    speaking_value_0_100:
      clampScore(data?.speaking_value_0_100) ??
      clampScore(data?.listening_value_0_100) ??
      speaking,
  };
}
...
export async function setStudentListeningOverride({
  ...
  return setStudentSpeakingOverride({
    db,
    actorId,
    userId,
    level,
    speakingValue: listeningValue,
  });
}
...
export async function closeStudentLevel({
  ...
  const [profileData, historyRows, gradeRows, overrideRows, progressRows] = await Promise.all([
    ...
    loadStudentLevelHistoryRowsByUserIds(db, [studentId], { includeId: true }),
    loadRowsByUserIdsWithFallback({
      db,
      table: "student_course_grades",
      columns: "user_id, level, admin_grade_0_100, comment, updated_at",
    ...
    loadStudentSkillOverridesByUserIds(db, [studentId]),
    loadProgressRowsByUserIds(db, [studentId]),
  ]);

  const skillMap = buildSkillAverageMap(progressRows, resolvedLevel);
  const speakingRow = pickRowByLevel(overrideRows, resolvedLevel);
  const speaking = resolveManualSpeakingValue(speakingRow) ?? clampScore(skillMap.speaking);
  const listening = clampScore(skillMap.listening);
  const vocabulary = clampScore(skillMap.vocabulary);
  ...
  const payload = {
    ...
    final_speaking_0_100: speaking ?? 0,
    ...
    final_vocabulary_0_100: vocabulary ?? 0,
  ...
  for (const missingColumn of getStudentSkillsMissingColumnsCache().get("student_level_history") || []) {
    if (Object.prototype.hasOwnProperty.call(workingPayload, missingColumn)) {
      delete workingPayload[missingColumn];
    }
  }
  let selectColumns = filterKnownMissingColumns("student_level_history", [
    "id",
    "user_id",
    "level",
    "started_at",
  ...
  for (let attempt = 0; attempt < 6; attempt += 1) {
    const result = await db
      .from("student_level_history")
      .upsert(workingPayload, { onConflict: "user_id,level" })
    ...
    if (!error) break;
    const missingColumn = getMissingColumnFromError(error);
    if (missingColumn && (Object.prototype.hasOwnProperty.call(workingPayload, missingColumn) || selectColumns.includes(missingColumn))) {
      rememberMissingColumn("student_level_history", missingColumn);
      if (Object.prototype.hasOwnProperty.call(workingPayload, missingColumn)) {
        delete workingPayload[missingColumn];
      }
...
export async function loadStudentAppSkillSnapshot({
  ...
  if (!studentId) {
    return {
      current: { speaking: null, reading: null, grammar: null, listening: null, vocabulary: null },
      historical: { speaking: null, reading: null, grammar: null, listening: null, vocabulary: null, grade: null, count: 0 },
      manual: { speaking: null },
      combined: { speaking: null, reading: null, grammar: null, listening: null, vocabulary: null },
    };
  }
  ...
  const [progressRows, historyRows, overrideRows] = await Promise.all([
    loadProgressRowsByUserIds(db, [studentId]),
    loadStudentLevelHistoryRowsByUserIds(db, [studentId]),
    loadStudentSkillOverridesByUserIds(db, [studentId]),
  ]);
