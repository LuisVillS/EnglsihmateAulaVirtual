...
export async function updateCrmLeadContactDetails(
  ...
  if (changedByUserId) {
    await db.from("crm_interactions").insert({
      ...
      direction: "system",
      operator_user_id: changedByUserId,
      summary: "Contact details updated",
      notes: "Manual contact fields updated from CRM quick edit.",
      call_outcome: null,
      metadata: {
        ...(metadata || {}),
        reason: normalizeFreeText(reason) || "manual_contact_update",
        changed_fields: {
          full_name: hasNameInput,
          email: hasEmailInput,
          phone: hasPhoneInput,
        },
      },
...
export async function createCrmLeadNote(
  ...
  if (!normalizedNote) {
    throw new Error("createCrmLeadNote requires note content.");
  }
  ...
  const { data, error } = await db
    ...
    .insert({
      ...
      summary: normalizeFreeText(summary) || normalizedNote.slice(0, 120),
      notes: normalizedNote,
      call_outcome: null,
      metadata: metadata || {},
      created_at: toIsoTimestamp(),
    })
    .select(CRM_INTERACTION_COLUMNS)
    .maybeSingle();
...
export async function moveCrmLeadStage(
  ...
  if (lead.current_stage_id !== stage.id) {
    const { error: historyError } = await db.from("crm_stage_history").insert({
      ...
      metadata: {
        ...
        brevo_template_id: stage.brevo_template_id || null,
  ...
  const noteInteraction = noteText
    ? await createCrmLeadNote(client, {
        leadId,
        note: noteText,
        summary: `Stage moved to ${stage.name}`,
