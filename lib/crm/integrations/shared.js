...
function buildCanonicalPhoneLeadFields(phoneInfo) {
  ...
  return {
    phone_country_code: phoneInfo.phoneCountryCode || null,
    phone_national_number: phoneInfo.phoneNationalNumber || null,
    phone_e164: phoneInfo.phoneE164 || null,
    phone_dialable: phoneInfo.phoneE164 || null,
    phone_validation_status: phoneInfo.isValid ? "valid" : "invalid",
    phone_validation_reason: phoneInfo.validationErrors?.[0] || null,
    phone_raw_input: phoneInfo.phone || null,
  };
}
...
export function buildRawSourceMetadata(normalized) {
  return {
    ...
    canonical_phone: normalized.phoneInfo
      ? {
          country_code: normalized.phoneInfo.phoneCountryCode || null,
          national_number: normalized.phoneInfo.phoneNationalNumber || null,
          e164: normalized.phoneInfo.phoneE164 || null,
          dialable: normalized.phoneInfo.phoneE164 || null,
          validation_status: normalized.phoneInfo.isValid ? "valid" : "invalid",
          validation_reason: normalized.phoneInfo.validationErrors?.[0] || null,
          raw_input: normalized.phoneInfo.phone || null,
        }
      : null,
...
function buildLeadPhonePayload(normalized) {
  if (normalized.phoneInfo?.isValid && normalized.phoneInfo?.phoneE164) {
    return {
      phone: normalized.phoneInfo.phoneE164,
    };
  }
