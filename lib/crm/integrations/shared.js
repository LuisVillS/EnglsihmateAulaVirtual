...
import { isExternalIngestMergeCandidate } from "./external-lead-merge-policy.js";
...
function buildCanonicalPhoneLeadFields(phoneInfo) {
  const canonicalPhone = getCanonicalPhoneInfo(phoneInfo);
  if (!canonicalPhone) return {};

  return {
    phone_country_code: canonicalPhone.phoneCountryCode || null,
    phone_national_number: canonicalPhone.phoneNationalNumber || null,
    phone_e164: canonicalPhone.phoneE164 || null,
    phone_dialable: canonicalPhone.phoneE164 || null,
    phone_validation_status: canonicalPhone.isValid ? "valid" : "invalid",
    phone_validation_reason: canonicalPhone.validationErrors?.[0] || null,
    phone_raw_input: canonicalPhone.phone || null,
  };
}

function normalizeCanonicalPhoneCountryCode(value) {
  const normalized = normalizeFreeText(value);
...
function getCanonicalPhoneInfo(phoneInfo) {
  if (!phoneInfo) return null;

  const validationErrors = Array.isArray(phoneInfo.validationErrors)
    ? phoneInfo.validationErrors.filter(Boolean)
    : phoneInfo.validationReason
      ? [phoneInfo.validationReason]
      : [];

  const isValid =
    typeof phoneInfo.isValid === "boolean"
      ? phoneInfo.isValid
      : phoneInfo.validationStatus
        ? phoneInfo.validationStatus === "valid"
        : Boolean(phoneInfo.phoneE164 || phoneInfo.e164);

  return {
    phone: phoneInfo.phone || phoneInfo.rawInput || null,
    phoneCountryCode: normalizeCanonicalPhoneCountryCode(
      phoneInfo.phoneCountryCode || phoneInfo.countryCode
    ),
    phoneNationalNumber: normalizeFreeText(
      phoneInfo.phoneNationalNumber || phoneInfo.nationalNumber
    )?.replace(/\D/g, "") || null,
    phoneE164: normalizeFreeText(phoneInfo.phoneE164 || phoneInfo.e164 || phoneInfo.dialable),
    isValid,
    validationErrors,
  };
}
...
export function buildRawSourceMetadata(normalized) {
  const canonicalPhone = getCanonicalPhoneInfo(normalized.phoneInfo);
  return {
    ...
    canonical_phone: canonicalPhone
      ? {
          country_code: canonicalPhone.phoneCountryCode || null,
          national_number: canonicalPhone.phoneNationalNumber || null,
          e164: canonicalPhone.phoneE164 || null,
          dialable: canonicalPhone.phoneE164 || null,
          validation_status: canonicalPhone.isValid ? "valid" : "invalid",
          validation_reason: canonicalPhone.validationErrors?.[0] || null,
          raw_input: canonicalPhone.phone || null,
        }
      : null,
...
export function normalizeManualCrmLeadInput(input = {}) {
  ...
  const sourceMetadata =
    input.sourceMetadata && typeof input.sourceMetadata === "object"
      ...
      : buildRawSourceMetadata({
          ...
          email,
          phone: phoneInfo?.phoneE164 || rawPhone || null,
          phoneInfo,
  ...
  return {
    ...
    email,
    fullName,
    phone: phoneInfo?.phoneE164 || normalizeFreeText(rawPhone),
    phoneInfo,
    siteKey: normalizeFreeText(input.siteKey),
    host: normalizeFreeText(input.host),
    formKey: normalizeFreeText(input.formKey),
...
async function findExistingExternalLead(service, { email, phone }) {
  ...
  if (normalizedPhone) {
    ...
    if (isExternalIngestMergeCandidate(phoneMatch)) {
      return phoneMatch;
    }
  }
  ...
  return rows.find(isExternalIngestMergeCandidate) || null;
}

function buildLeadPhonePayload(normalized) {
  const canonicalPhone = getCanonicalPhoneInfo(normalized.phoneInfo);
  if (canonicalPhone?.isValid && canonicalPhone?.phoneE164) {
    return {
