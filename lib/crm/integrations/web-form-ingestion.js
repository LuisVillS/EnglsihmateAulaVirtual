...
import { shouldSkipTurnstileValidationForHost, verifyTurnstileToken } from "@/lib/turnstile/verify";
...
export async function submitCrmWebFormLead({
  ...
  const shouldSkipTurnstile = skipTurnstile || shouldSkipTurnstileValidationForHost(host);
  ...
  const inboundEvent = await recordCrmInboundEvent(crmService, {
    ...
    payload,
    headers: headerJson,
    validationStatus: shouldSkipTurnstile ? "skipped" : "pending",
    processingStatus: "pending",
  });

  let turnstileVerified = Boolean(shouldSkipTurnstile);
  ...
  try {
    ...
    await markCrmInboundEventProcessed(crmService, inboundEvent?.id, {
      validationStatus: shouldSkipTurnstile ? "skipped" : "valid",
      processingStatus: "processed",
      turnstileValid: shouldSkipTurnstile ? null : true,
    ...
    if (inboundEvent?.id) {
      await markCrmInboundEventFailed(crmService, inboundEvent.id, {
        validationStatus: shouldSkipTurnstile ? "skipped" : turnstileVerified ? "valid" : "invalid",
        processingStatus: "failed",
