...
export function shouldSendBrevoAutomationForLead(automation = {}, lead = {}) {
  ...
  if (!rules.include.length && !rules.exclude.length) {
    return { shouldSend: true, reason: null, leadOrigins, rules };
  }

  if (rules.include.length && !leadOrigins.some((origin) => rules.include.includes(origin))) {
    return { shouldSend: false, reason: "source_not_included", leadOrigins, rules };
  }

  if (rules.exclude.length && leadOrigins.some((origin) => rules.exclude.includes(origin))) {
    return { shouldSend: false, reason: "source_excluded", leadOrigins, rules };
  }
