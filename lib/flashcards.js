...
export function mapLibraryFlashcardRow(row) {
  return {
    ...
    cefrLevel: String(row?.cefr_level || row?.cefrLevel || "").trim().toUpperCase(),
    themeTag: String(row?.theme_tag || row?.themeTag || "").trim().toLowerCase(),
...
export function resolveAssignedFlashcardRow(row, flashcardsById = new Map(), fallbackOrder = 1) {
  ...
  return {
    ...
    cefrLevel: String(source?.cefr_level || source?.cefrLevel || "").trim().toUpperCase(),
    themeTag: String(source?.theme_tag || source?.themeTag || "").trim().toLowerCase(),
...
export function parseFlashcardsBatch(rawBatch) {
  ...
  return rows.map((row, index) => ({
    ...
    cefrLevel: String(row?.cefrLevel || row?.cefr_level || "").trim().toUpperCase(),
    themeTag: String(row?.themeTag || row?.theme_tag || "").trim().toLowerCase(),
