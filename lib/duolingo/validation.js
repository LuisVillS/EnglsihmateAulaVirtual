...
function normalizeAudioMatch(raw, errors) {
  ...
  if (!questions.length) {
    errors.push("Listening Exercise requiere al menos 1 pregunta.");
  }

  questions.forEach((question, index) => {
    if (!cleanText(question?.prompt)) {
      errors.push(`Listening Exercise pregunta ${index + 1} requiere 'prompt'.`);
    }

    if (question?.type === LISTENING_QUESTION_TYPES.MULTIPLE_CHOICE) {
      const options = cleanArray(question?.options);
      if (options.length !== 4) {
        errors.push(`Listening Exercise pregunta ${index + 1} (multiple_choice) requiere exactamente 4 opciones.`);
      }
      const correctIndex = toInteger(question?.correct_index);
      if (correctIndex == null || correctIndex < 0 || correctIndex >= 4) {
        errors.push(`Listening Exercise pregunta ${index + 1} (multiple_choice) requiere 'correct_index' valido.`);
      }
      return;
    }
    ...
    if (question?.type === LISTENING_QUESTION_TYPES.WRITTEN) {
      ...
      if (!acceptedAnswers.length) {
        errors.push(`Listening Exercise pregunta ${index + 1} (written) requiere al menos una respuesta valida.`);
      }
      return;
    }
    ...
    if (question?.type === LISTENING_QUESTION_TYPES.TRUE_FALSE) {
      if (typeof question?.correct_boolean !== "boolean") {
        errors.push(`Listening Exercise pregunta ${index + 1} (true_false) requiere 'correct_boolean'.`);
      }
    }
  });
