...
import { buildListeningQuestionsFromContent, summarizeListeningQuestionResults } from "@/lib/listening-exercise";
  ...
  if (questions.length) {
    const answersById =
      answer?.questions && typeof answer.questions === "object"
        ? answer.questions
        : {};
    const summary = summarizeListeningQuestionResults(questions, answersById);
    return summary.total > 0 && summary.complete && summary.correctCount === summary.total;
  }
...
function evaluateReadingExercise(content, answer) {
  const questions = buildListeningQuestionsFromContent(content, { includeFallback: false });
...
export function evaluateExerciseAnswer({ type, content, answer }) {
  ...
  switch (normalizedType) {
    ...
    case EXERCISE_TYPES.READING_EXERCISE:
      return evaluateReadingExercise(content, answer);
