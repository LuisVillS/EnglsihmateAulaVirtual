...
async function loadStudentProfileById(client, profileId) {
  const columns = [
    "id, student_code, id_document, dni, full_name, email, role, status, xp_total, current_streak",
    "id, student_code, dni, full_name, email, role, status, xp_total, current_streak",
    "id, student_code, dni, full_name, email, role, status",
  ];
...
export async function upsertStudentByCode({
  ...
  const client = serviceClient || (hasServiceRoleClient() ? getServiceSupabaseClient() : null);
  if (!client) {
  ...
  if (existing?.id) {
    ...
    if (normalizedName) {
      ...
      updatePayload.first_name = nameParts.first;
      updatePayload.last_name = nameParts.last;
    }
