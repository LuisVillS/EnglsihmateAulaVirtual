...
function isMissingAuthRateLimitTable(error) {
  const message = String(error?.message || "").toLowerCase();
  return message.includes("auth_rate_limits") && (message.includes("does not exist") || message.includes("schema cache"));
}
...
async function loadRateLimitRecord(service, scopeKey) {
  ...
  if (error) {
    if (isMissingAuthRateLimitTable(error)) {
      return null;
    }
