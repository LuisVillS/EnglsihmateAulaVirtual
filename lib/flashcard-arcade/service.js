...
async function loadSavedDeckDrafts(db, { deckId = null, userId = "", courseLevel = "" } = {}) {
  const cefrLevel = normalizeStudentCefrLevel(courseLevel);
  let sharedDeckQuery = db
    .from("flashcard_decks")
    .select("id, title, description, cover_image_url, source_type, cefr_level, theme_tag, scenario_tag, metadata, owner_user_id, updated_at")
    .eq("is_active", true)
    .is("owner_user_id", null)
    .order("updated_at", { ascending: false });

  if (deckId) {
    sharedDeckQuery = sharedDeckQuery.eq("id", deckId);
  } else if (cefrLevel) {
    sharedDeckQuery = sharedDeckQuery.eq("cefr_level", cefrLevel);
  }

  let studentDeckQuery = null;
  if (userId) {
    studentDeckQuery = db
      .from("flashcard_decks")
      .select("id, title, description, cover_image_url, source_type, cefr_level, theme_tag, scenario_tag, metadata, owner_user_id, updated_at")
    ...
    if (deckId) {
      studentDeckQuery = studentDeckQuery.eq("id", deckId);
    }
  }

  const [sharedDeckResult, studentDeckResult] = await Promise.all([
    sharedDeckQuery,
    studentDeckQuery,
  ]);

  if (sharedDeckResult.error) {
    throw new Error(sharedDeckResult.error.message || "No se pudieron cargar los decks de flashcards.");
  }
  if (studentDeckResult?.error) {
    throw new Error(studentDeckResult.error.message || "No se pudieron cargar tus decks personalizados.");
  }

  const decks = mergeDeckRows(sharedDeckResult.data, studentDeckResult?.data);
  if (!decks.length) {
    return [];
  }
  ...
  return decks
    .map((deck) => {
      ...
      return {
        ...
        sourceLabel:
          String(deck?.owner_user_id || "").trim() === String(userId || "").trim()
            ? "My deck"
            : formatStudentThemeLabel(deck?.theme_tag) || "Level deck",
        cefrLevel: String(deck?.cefr_level || "").trim().toUpperCase(),
        themeTag: String(deck?.theme_tag || "").trim().toLowerCase(),
        ownerUserId: String(deck?.owner_user_id || "").trim() || null,
        isEditable: String(deck?.owner_user_id || "").trim() === String(userId || "").trim(),
        cards,
...
function toDeckSummary(deck) {
  return {
    ...
    isEditable: deck?.isEditable === true,
...
function sortDeckDrafts(decks = []) {
  const sourcePriority = {
    ...
    [FLASHCARD_DECK_SOURCE_TYPES.STUDENT]: 2,
    [FLASHCARD_DECK_SOURCE_TYPES.SESSION]: 3,
  };
...
async function loadAllDeckDrafts(db, { userId, courseLevel = "" } = {}) {
  const [sessionDecks, savedDecks] = await Promise.all([
    ...
    loadSavedDeckDrafts(db, { userId, courseLevel }),
  ]);
