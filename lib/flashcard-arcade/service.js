...
async function loadSessionDeckDrafts(db, { userId, sessionId = null } = {}) {
...
async function loadSavedDeckDrafts(db, { deckId = null } = {}) {
  let deckQuery = db
...
async function loadAllDeckDrafts(db, { userId } = {}) {
  const [sessionDecks, savedDecks] = await Promise.all([
    loadSessionDeckDrafts(db, { userId }),
    loadSavedDeckDrafts(db),
  ]);

  return [...sessionDecks, ...savedDecks];
}

export async function loadFlashcardArcadeHubData(db, { userId, legacyXpTotal = 0 } = {}) {
  const gamification = await ensureGamificationProfile(db, {
    userId,
    legacyXpTotal,
  });

  const deckDrafts = await loadAllDeckDrafts(db, { userId });
  ...
  const [recentSession, competition] = await Promise.all([
    loadRecentFlashcardSession(db, userId),
...
export async function loadFlashcardDeck(db, { userId, deckKey } = {}) {
  ...
  if (parsed.kind === FLASHCARD_DECK_KEY_PREFIXES.SESSION) {
    ...
    selectedDecks = await loadAllDeckDrafts(db, { userId });
  }
