...
import {
  ...
  resolveUnifiedCoursePrice,
} from "@/lib/course-config";
import { assertValidCrmPhoneInput } from "@/lib/crm/phones";
...
function parseBirthDate(value) {
  ...
  const birthDate = parsed.toISOString().slice(0, 10);
  const today = getLimaTodayISO();
  const [year, month, day] = today.split("-").map(Number);
  const minimumDate = new Date(Date.UTC(year - MINIMUM_REGISTER_AGE, month - 1, day))
    .toISOString()
    .slice(0, 10);
  if (birthDate > minimumDate) return null;
  return birthDate;
}
...
function normalizeCourseType(value) {
  return normalizeUnifiedCourseType(value);
}
...
export async function upsertPreEnrollmentProfile({
  email,
  fullName,
  phone,
  phoneCountryCode,
  ...
  const normalizedPhone = assertValidCrmPhoneInput(
    {
      phone,
      phoneCountryCode,
      phoneNationalNumber,
  ...
  if (!parsedBirth) {
    throw new Error(`La fecha de nacimiento es invalida. Debes tener al menos ${MINIMUM_REGISTER_AGE} anos.`);
  }
...
export async function verifyEmailOtp({ userId, code }) {
  ...
  if (expired) {
    throw new Error("Codigo de Acceso expirado.");
  }
...
export async function getPreEnrollment(userId) {
  ...
  return {
    ...preEnrollment,
    selected_course_type: normalizeCourseType(preEnrollment.selected_course_type),
  ...
  for (let attempt = 0; attempt < 8; attempt += 1) {
    ...
    if (!result.error) {
      return result.data
        ? {
            ...result.data,
            selected_course_type: normalizeCourseType(result.data.selected_course_type),
            price_total: resolveUnifiedCoursePrice(result.data.price_total),
          }
        : result.data;
    }
    const missingColumn = getMissingColumnFromError(result.error);
