...
export const LISTENING_WRITTEN_ANSWER_MODES = {
  PLAIN_INPUT: "plain_input",
  BLANK_INPUT: "blank_input",
};

const LISTENING_WRITTEN_ANSWER_MODE_VALUES = Object.values(LISTENING_WRITTEN_ANSWER_MODES);
...
export function normalizeListeningWrittenAnswerMode(value) {
  const raw = cleanText(value).toLowerCase().replace(/\s+/g, "_");
  return LISTENING_WRITTEN_ANSWER_MODE_VALUES.includes(raw)
    ? raw
    : LISTENING_WRITTEN_ANSWER_MODES.PLAIN_INPUT;
}
...
export function createDefaultListeningQuestion(type = LISTENING_QUESTION_TYPES.MULTIPLE_CHOICE, index = 0) {
  ...
  if (resolvedType === LISTENING_QUESTION_TYPES.WRITTEN) {
    return {
      ...
      written_answer_mode: LISTENING_WRITTEN_ANSWER_MODES.PLAIN_INPUT,
      example_answer: "",
      alternate_answers: [],
      accepted_answers: [""],
      explanation: "",
...
export function normalizeListeningQuestion(question, index = 0, options = {}) {
  ...
  const prompt = promptValue !== undefined
    ...
    : fallback.prompt;
  const explanation = readText(
    source.explanation ?? source.explanation_richtext ?? source.explanationRichText ?? fallback.explanation ?? "",
    options
  );

  if (type === LISTENING_QUESTION_TYPES.WRITTEN) {
    const writtenAnswerMode = normalizeListeningWrittenAnswerMode(
      source.written_answer_mode ?? source.writtenAnswerMode
    );
    ...
    const explicitAlternateAnswers = Array.isArray(source.alternate_answers)
      ? source.alternate_answers
      : (Array.isArray(source.alternateAnswers) ? source.alternateAnswers : null);

    const acceptedAnswersFromSource = explicitAcceptedAnswers
      ? uniqueTextList(explicitAcceptedAnswers, options)
      : uniqueTextList([
        source.example_answer,
        source.exampleAnswer,
        ...(explicitAlternateAnswers || []),
    ...
    const acceptedAnswers = acceptedAnswersFromSource
      .map((item) => readText(item, options))
      .filter((item) => (options.preserveDraftText ? item != null : Boolean(item)));
    if (!acceptedAnswers.length) {
      acceptedAnswers.push(...fallback.accepted_answers);
    }

    const explicitExampleAnswer = readText(source.example_answer ?? source.exampleAnswer, options);
    const exampleAnswer = explicitExampleAnswer || readText(acceptedAnswers[0], options);
    let alternateAnswers = explicitAlternateAnswers
      ? uniqueTextList(explicitAlternateAnswers, options)
      : acceptedAnswers.slice(exampleAnswer ? 1 : 0);

    alternateAnswers = alternateAnswers
      .map((item) => readText(item, options))
      .filter((item) => (options.preserveDraftText ? item != null : Boolean(item)));

    const mergedAcceptedAnswers = uniqueTextList([
      exampleAnswer,
      ...alternateAnswers,
      ...acceptedAnswers,
    ], options).filter((item) =>
      options.preserveDraftText ? item != null : Boolean(item)
    );
    ...
    return {
      ...
      type,
      prompt,
      written_answer_mode: writtenAnswerMode,
      example_answer: exampleAnswer,
      alternate_answers: alternateAnswers,
      accepted_answers: mergedAcceptedAnswers.length ? mergedAcceptedAnswers : fallback.accepted_answers,
      explanation,
    };
  }
...
export function getListeningQuestionCorrectAnswerText(question) {
  ...
  if (resolvedQuestion.type === LISTENING_QUESTION_TYPES.WRITTEN) {
    return cleanText(
      resolvedQuestion.example_answer ||
      (Array.isArray(resolvedQuestion.accepted_answers) ? resolvedQuestion.accepted_answers[0] : "")
    ) || "-";
  }
  if (resolvedQuestion.type === LISTENING_QUESTION_TYPES.TRUE_FALSE) {
