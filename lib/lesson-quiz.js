...
export function canShowCorrectionDetails({
  ...
  if (Boolean(isFullyCorrect)) return true;
  if (Number(attemptsRemaining) <= 0) return true;
  if (Boolean(deadlinePassed)) return true;

  if (Boolean(isPartiallyIncorrect) || Boolean(isIncorrect)) return false;
  return false;
}
