...
export async function recordLibraryReadOpen({ db, userId, libraryBookId }) {
  ...
  const touchUserState = async () =>
    db.from("library_book_user_state").upsert(
      {
        user_id: safeUserId,
  ...
  if (withinSessionWindow) {
    ...
    const { error: userStateError } = await touchUserState();
    if (userStateError) {
      throw new Error(userStateError.message || "No se pudo actualizar el estado de lectura.");
    }
...
export async function loadAdminLibraryOverview({ db }) {
  ...
  return {
    counts: {
      ...
      pendingReview: staging.filter((row) => ["pending", "needs_review"].includes(row.ingestionStatus)).length,
...
export async function loadAdminDuplicateGroups({ db }) {
  ...
  return detectDuplicateGroups([
    ...
    ...staging
      .filter((record) => record.ingestionStatus !== "rejected")
      .map((record) => ({
        ...record,
        title: record.rawTitle,
        recordScope: "staging",
...
export function buildLibraryBookPayloadFromCandidate(candidate, overrides = {}) {
  ...
  const authorDisplay = normalizeWhitespace(overrides.authorDisplay || candidate.authorDisplay || "");
  ...
  return {
    ...
    source_name: cleanText(candidate.sourceName || candidate.source_name) || "openlibrary",
    ...
    ebook_access: cleanText(candidate.ebookAccess || candidate.ebook_access).toLowerCase() || null,
    has_fulltext: Boolean(candidate.hasFulltext ?? candidate.has_fulltext),
    readable_online: Boolean(candidate.readableOnline ?? candidate.readable_online),
    preview_only: Boolean(candidate.previewOnly ?? candidate.preview_only),
