...
export function canUseLibraryReaderArrowKeys(event) {
  ...
  return canUseLibraryReaderShortcut(event);
}

export function canUseLibraryReaderPlaybackKey(event) {
  const key = cleanText(event?.key);
  const code = cleanText(event?.code);
  if (![" ", "Spacebar", "Space"].includes(key) && code !== "Space") return false;
  return canUseLibraryReaderShortcut(event);
}

function canUseLibraryReaderShortcut(event) {
...
export function playLibraryPageFlipSound({
  ...
  rustleSource.buffer = createPageNoiseBuffer(context, durationSeconds * 0.78);
  ...
  rustleLowPass.frequency.setValueAtTime(2400 + Math.random() * 220, now);
  rustleLowPass.Q.value = 0.5;

  const rustleHighPass = context.createBiquadFilter();
  rustleHighPass.type = "highpass";
  rustleHighPass.frequency.setValueAtTime(860 + Math.random() * 120, now);
  rustleHighPass.Q.value = 0.55;
  ...
  rustleGain.gain.exponentialRampToValueAtTime(volume * 0.72, now + 0.02);
  rustleGain.gain.exponentialRampToValueAtTime(0.0001, now + durationSeconds * 0.82);
  ...
  rustleSource.connect(rustleLowPass);
  rustleLowPass.connect(rustleHighPass);
  rustleHighPass.connect(rustleGain);
  rustleGain.connect(context.destination);

  noiseSource.start(now);
  rustleSource.start(now + 0.014);
  noiseSource.stop(now + durationSeconds);
  rustleSource.stop(now + durationSeconds * 0.82);
  return true;
}
