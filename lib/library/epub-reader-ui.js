...
export function resolveLibraryEpubPageKind(href = "") {
  ...
  if (/^titlepage\.xhtml$/i.test(fileName)) return "title-leaf";
...
export function resolveLibraryEpubDisplayMode({
  ...
  const numericIndex = Number(locationIndex);
  const isFirstBookPage =
    Boolean(location?.atStart) ||
    (Number.isFinite(numericIndex) && numericIndex <= 0);

  if (pageKind === "title-leaf" && isFirstBookPage) {
    return "single";
  }
...
export function resolveLibraryEpubVisiblePageNumbers({
  ...
  if (displayMode === "single") {
    return {
      left: Number.isFinite(numericPageNumber) && numericPageNumber > 0 ? numericPageNumber : null,
      right: null,
    };
  }
  ...
  return {
    left: numericPageNumber,
    right: numericPageNumber + 1,
  };
}
