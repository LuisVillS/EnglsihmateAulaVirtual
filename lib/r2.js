...
export async function assertR2Writable() {
  if (r2WritableCheckPromise) {
    return r2WritableCheckPromise;
  }

  r2WritableCheckPromise = (async () => {
    const key = `_healthchecks/r2-write-check-${Date.now()}.txt`;
    try {
      await putObjectToR2(key, Buffer.from("ok"), "text/plain");
      await deleteObjectFromR2(key);
      return true;
    } catch (error) {
      r2WritableCheckPromise = null;
      throw new Error(
        `R2 no permite escritura en el bucket configurado (${R2_BUCKET}). Verifica bucket, endpoint y permisos Object Write del token. Detalle: ${error?.message || "Access denied"}`
      );
    }
  })();
